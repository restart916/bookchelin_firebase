<template>
  <div>
    ...
    <PullToRefresh
      ...
      <div id="viewer2" class="viewer2 scrolled" ref="viewer2"></div>
...
export default {
  ...
  methods: {
    test(e) {
      console.log('test', e, e.target.scrollTop)
    ...
    initRendition() {
      ...
      this.displayed.then((renderer) => {
        // -- do stuff
        console.log('this.displayed.then', renderer);
        setTimeout(() => {
            this.loading = true;
        }, 200);
      });
      ...
      this.rendition.on("rendered", (section) => {
        ...
        var $viewer = document.getElementById("viewer2");
        $viewer.innerHTML = section.output;
        ...
        if (this.isShow) {
          // window.scrollTo(0,0);
          this.$nextTick(() => {
            var container = document.querySelector("#viewer2");
            console.log(container == this.$refs.viewer);
            console.log(container.scrollTop, container.scrollHeight);

            console.log('this.test', this.test, this.test.scrollTop);
            if (this.scrollBody) {
