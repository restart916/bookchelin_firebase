<template>
  <div>
    ...
    <section class="section">
      1. 추가하기 - log_select_id 를 제외한 데이터를 입력하고 add 버튼을
      누른다<br />
      2. 삭제하기 - 아래 목록에서 삭제버튼을 누른다<br />
      3. 수정하기 - 아래 목록에서 수정하기 버튼을 누른다. 위에 입력란에 데이터가
      채워지면 수정하고 다시 add 버튼을 누른다<br />
    ...
    <section class="section">
      ...
      <div>
        <div>image_url</div>
        <div class="control">
          <textarea type="text" class="input" v-model="image_url"> </textarea>
        </div>
      </div>
      <div class="Column">
    ...
    <section class="section">
      <div class="container">
        <div
          class="columns"
          v-for="log_select in log_selects"
          :key="log_select['.key']"
        >
          <div
            class="notification Column"
            :class="{ active: !log_select.hidden }"
          >
            <div>
            ...
            <div class="button" @click="selectLogSelect(log_select['.key'])">
              수정하기
            </div>
            <div class="button" @click="deleteLogSelect(log_select['.key'])">
              삭제
            </div>
            <div v-if="log_select.hidden">
              <div class="button" @click="showLogSelect(log_select['.key'])">
                보여주기
              </div>
            </div>
            <div v-else>
              <div class="button" @click="hideLogSelect(log_select['.key'])">
                숨기기
              </div>
            </div>
            <select
              v-model="log_select.category"
              @change="onChangeBookCategory(log_select)"
            >
              <option disabled value="0">선택해주세요</option>
              <option
                v-for="category in book_category"
                :key="category.key"
                :value="category.id"
              >
                {{ category.name }}
...
export default {
  ...
  firestore() {
    return {
      log_selects: firestore
        .collection('log_select')
        .orderBy('timestamp', 'desc'),
      log_select_click: firestore.collection('log_select_click'),
      book_category: firestore
        .collection('book_category')
        .orderBy('id', 'desc'),
    };
  },
  mounted() {
  ...
  methods: {
    // async onChangeUrl() {
    //   console.log('onChangeUrl', this.link_url);

    //   try {
    //     const url = new URL(this.link_url);
    ...
    selectLogSelect(key) {
      for (let log_select of this.log_selects) {
        if (log_select['.key'] == key) {
          ...
          this.image_url = log_select['image_url'];
