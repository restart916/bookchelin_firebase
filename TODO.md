...
- [x] 책 상세 SSR JSON-LD 보강 — BreadcrumbList 추가(2026-06-13), 도배 리뷰가 aggregateRating 오염하던 것 제외 처리. 기존 Book/Offer/aggregateRating 은 이미 충실
...
- [x] 도배 리뷰 정리 완료 (2026-06-13) — `scripts/clean_spam_reviews.js` 로 206건 hide:"1" 처리(빈 리뷰·자모/기호만, 오탐 0). 필드는 기존 Android 규약 `hide:"1"`(어드민 원본 is_hide 와 불일치였던 것 통일). Android 1.00.87 즉시 반영, Flutter 는 리뷰 미표시라 해당없음, web_book/SEO 도 제외 반영
- [x] 어드민 리뷰 페이지(EditReviewView) 개선 — 책 제목 표시 + 숨기기 토글 복구(기존 미작동) + 검색필터
- [x] Flutter(iOS) 책 상세에 독자 리뷰 표시 추가 (2026-06-13, bookchelin_flutter `df9a7e2`) — 그동안 책소개만 보였음. hide:"1" 도배 제외 + 평점 헤더. 다음 iOS 배포(1.1.2)에 포함
...
### iOS 크래시 수정 `[flutter(iOS)]`
- [x] **현황 확인 (2026-06-13): Crashlytics 미해결 크래시 0건, crash-free 100%(7일).** 추측성으로 고칠 명확한 대상 없음 — 아래 리뷰 증상들은 1.1.1 이전 버전이거나 크래시 아닌 렌더 실패로 판단
- [~] 검색창 입력 시 튕김 — book_search_page 코드 점검: mounted 가드·dispose 정리 견고, 명백한 버그 없음. 옛 버전 추정 (1.1.1 모니터링)
- [~] 아이패드 첫 화면 후 튕김 — 1.1.1 의 아이패드 레이아웃 수정(`6ee93d0`)으로 개선됐을 가능성. 출시 후 재확인
- [~] 맥북 흰 화면 — Mac Catalyst(Designed for iPad) 렌더 이슈로 크래시 아님. WebView/EPUB 렌더 별도 검토 사안
